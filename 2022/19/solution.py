...
def getMaxGeode(time, production, stock, blueprint):
	...
	# build ore next 
	timeToResource = max(math.ceil((blueprint[0][0] - stock[0])/production[0]), 0)
	...
	# build clay next 
	timeToResource = max(math.ceil((blueprint[1][0] - stock[0])/production[0]), 0)
	waitTime = timeToResource + 1
	if waitTime < time:
		cantBuildAnything = False
		maxFound = max(
	...
	if production[1] > 0:
		timeToResource = max(
			max(math.ceil((blueprint[2][0] - stock[0])/production[0]), 0),
			max(math.ceil((blueprint[2][1] - stock[1])/production[1]), 0)
			)
		waitTime = timeToResource + 1
		if waitTime < time:
			cantBuildAnything = False
	...
	if production[2] > 0:
		timeToResource = max(
			max(math.ceil((blueprint[3][0] - stock[0])/production[0]), 0),
			max(math.ceil((blueprint[3][1] - stock[2])/production[2]), 0)
			)
		waitTime = timeToResource + 1
		if waitTime < time:
			cantBuildAnything = False
